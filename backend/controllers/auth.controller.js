...
export const verifyEmail = async (req, res) => {
    ...
    try {
        ...
        if (!user) {
            return res.status(400).json({ success: false, message: "Invalid or expried verification code" })
        }
...
export const forgotPassword = async (req, res) => {
    ...
    try {
        ...
        const resetTokenExpiresAt = Date.now() + 1 * 60 * 60 * 100; //1hour
...
export const resetPassword = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(400).json({success: false, message: "Invalid or exprired reset token"})
        }
